...
func flattenAwsLbTargetGroupStickiness(d *schema.ResourceData, attributes []*elbv2.TargetGroupAttribute) error {
	...
	if !stickinessMap["enabled"].(bool) && stickinessMap["type"].(string) == "lb_cookie" && d.Get("protocol").(string) != elbv2.ProtocolEnumHttp && d.Get("protocol").(string) != elbv2.ProtocolEnumHttps {
		log.Printf("[WARN] invalid configuration, this will fail in a future version: stickiness enabled %v, protocol %s, type %s", stickinessMap["enabled"].(bool), d.Get("protocol").(string), stickinessMap["type"].(string))
	} else if err := d.Set("stickiness", setStickyMap); err != nil {
		return err
	}
	return nil
}
